...
def get_opportunity_details(request):
    ...
    return JsonResponse({
        ...
        'detail_url': f"/jobs/{opp_id}/" if opp_type == 'job' else (f"/scholarships/{opp_id}/" if opp_type == 'scholarship' else (f"/schemes/{opp_id}/" if opp_type == 'scheme' else (f"/skills/{opp_id}/" if opp_type == 'skill' else f"/business/{opp_id}/"))),
...
def apply_opportunity(request):
    ...
    if request.user.is_authenticated:
        ...
        if key in session_applied:
            ...
            if not existing_app:
                existing_app = Application(opportunity_name=opp_name, id=opp_id)
        else:
            existing_app = None
